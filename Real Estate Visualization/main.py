...
for _, row in data.iterrows():
    normalize_price = (row['MedHouseVal'] - price_min) / (price_max - price_min)
    color = plt.cm.RdYlGn(1 - normalize_price)

    normalize_rooms = (row['AveRooms'] - size_min) / (size_max - size_min)
